			...
			if($arrstr_args[1] == "get"){
				if(count($arrstr_args) != 4){
					...
					if($arrstr_args[2] == "pol"){
						Map::getPoliticalControl($arrstr_args[3]);
						Notifications::message("cmd", "political control of city ".$arrstr_args[3]." = TODO", []);
					}else{ // religios
						Map::getReligiosControl($arrstr_args[3]);
						Notifications::message("cmd", "religius control of city ".$arrstr_args[3]." = TODO", []);
					}
				}
			}else{ // set
				if(count($arrstr_args) != 5){
					Notifications::message("cmd-error", "map-command has format map [get|set] [pol|rel] cityId [value to set city to]", []);
				}else{
					if($arrstr_args[2] == "pol"){
						Map::setPoliticalControl(SpaceIDs::LONDON, Powers::OTTOMAN);
						Notifications::message("cmd", "set political control of city ".$arrstr_args[3]." to ".$arrstr_args[4], []);
					}else{ // religios
						Map::setPoliticalControl(SpaceIDs::LONDON, ReligionIDs::REFORMED);
						Notifications::message("cmd", "set religius control of city ".$arrstr_args[3]." to ".$arrstr_args[4], []);
					}
				}
			}
